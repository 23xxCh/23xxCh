  ...
  return (
    <div className="phase-list">
      {rows.map((row, idx) => {
        ...
        return (
          <div className="phase-item" key={`${phase}-${idx}-${row?.start_ts || ""}`}>
            <strong>{phase}</strong>
            <span>{duration}</span>
    ...
    <div className="page">
      <header className="hero">
        ...
        <div className="hero-right">
          <div className={statusClass}>{String(state).toUpperCase()}</div>
      ...
      {activeTab === "overview" ? (
        <>
          <section className="grid cards">
            <article className="card"><span>当前阶段</span><strong>{phase}</strong></article>
            <article className="card"><span>机器人模式</span><strong>{modeLabel(mode)}</strong></article>
            <article className="card"><span>气体浓度</span><strong>{String(gas)}</strong></article>
            <article className="card"><span>源点状态</span><strong>{String(sourceFound)}</strong></article>
          </section>
          ...
          <section className="grid control-grid">
            <article className="panel">
              ...
              <div className="btn-row">
                <button className="btn primary" onClick={startSim}>开始仿真</button>
                <button className="btn danger" onClick={stopSim}>停止仿真</button>
                <button className="btn" onClick={() => Promise.all([refreshStatus(), refreshMetrics(), refreshLogs()])}>刷新状态</button>
              </div>
            ...
            <article className="panel">
              ...
              <div className="gas-diagnostics">
                <div className="gas-diag-card">
                  ...
                  <strong className={`gas-signal gas-signal-${String(gasSignalStatus).toLowerCase()}`}>{gasSignalLabel(gasSignalStatus)}</strong>
                </div>
                <div className="gas-diag-card">
                  <span>原始读数</span>
      ...
      {activeTab === "heatmap" ? (
        <section className="panel heatmap-panel">
          ...
          <div className="heatmap-status">
            ...
            {heatmapData.error && (
              <span className="heatmap-error">{heatmapData.error}</span>
            )}
        ...
        <section className="panel">
          ...
          <div className="grid ai-grid">
            <div className="ai-card">
              ...
              <div className="btn-row">
                <button className="btn" onClick={saveLlmProfile}>保存配置</button>
                <button className="btn" onClick={loadLlmProfiles}>刷新配置</button>
                <button className="btn" onClick={activateProfile}>设为当前</button>
                <button className="btn" onClick={checkProfile}>连接测试</button>
              </div>
            </div>
            ...
            <div className="ai-card">
              ...
              <div className="btn-row">
                <button className="btn primary" onClick={sendAi}>发送给 AI</button>
                <button className="btn" onClick={runAiOnce}>执行 AI 单轮流程</button>
              </div>
              <div className="reply">{llmReply || "AI 分析结果会显示在这里。"}</div>
              <div className="actions">
                {llmActions.map((action, idx) => (
                  <div className="action-item" key={`${action.type}-${idx}`}>
                    <div>
                    ...
                    <button className="btn" onClick={() => executeAction(action)}>执行动作</button>
                  </div>
                ))}
              </div>
            </div>
