...
def get_h2track_bringup_share_path() -> Optional[Path]:
    ...
    Resolution order:
    1. H2TRACK_BRINGUP_SHARE environment variable
    2. ament_index_python if package is installed
    3. Computed from workspace root

    Returns:
        Path to install/h2track_bringup/share/h2track_bringup, or None if not found
    """
...
# Backward-compatible alias — old code may call get_h2track_sim_share_path()
def get_h2track_sim_share_path() -> Optional[Path]:
    """Alias for get_h2track_bringup_share_path(). Kept for backward compatibility."""
...
_h2track_bringup_share = get_h2track_bringup_share_path()
BASELINE_WORLD_PATH = (
    _h2track_bringup_share / "worlds" / "h2track_lab.world"
    if _h2track_bringup_share
    else WORKSPACE_ROOT / "install" / "h2track_bringup" / "share" / "h2track_bringup" / "worlds" / "h2track_lab.world"
)
WAREHOUSE_WORLD_PATH = (
    _h2track_bringup_share / "scenes" / "warehouse" / "warehouse.world"
    if _h2track_bringup_share
    else WORKSPACE_ROOT / "install" / "h2track_bringup" / "share" / "h2track_bringup" / "scenes" / "warehouse" / "warehouse.world"
)
...
def validate_paths() -> list[str]:
    ...
    bringup_share = get_h2track_bringup_share_path()
    if bringup_share is None or not bringup_share.exists():
        warnings.append(f"h2track_bringup share directory not found")
