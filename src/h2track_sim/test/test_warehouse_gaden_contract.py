...
WAREHOUSE_SCENE = WORKTREE_ROOT / 'src' / 'h2track_sim' / 'scenes' / 'warehouse' / 'scene.yaml'
...
def _warehouse_map_bounds() -> tuple[float, float, float, float]:
    scene = _warehouse_scene()
    map_yaml = _load_yaml(WORKTREE_ROOT / 'src' / 'h2track_sim' / scene['map'])
    width, height = _pgm_size((WORKTREE_ROOT / 'src' / 'h2track_sim' / scene['map']).with_name(map_yaml['image']))
