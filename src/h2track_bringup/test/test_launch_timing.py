"""Tests for launch file structure and parameter routing.

After the 3-layer launch hierarchy refactoring:
- bringup.launch.py: backward-compatible wrapper → delegates to robot.launch.py
- robot.launch.py: Layer 1, owns _PARAMS schema and scene resolution
- tracking.launch.py: Layer 2, owns bt_node_runner + particle_filter_node
- gas_simulation.launch.py: Layer 2, owns gas_field_node + GADEN pipeline
- visualization.launch.py: Layer 2, owns RViz
"""
...
def test_robot_launch_module_imports_without_launch_pythonpath_side_effects():
    module = _load_launch_module("robot.launch.py")
    assert hasattr(module, 'generate_launch_description')


def test_bringup_delegates_to_robot_launch():
    text = _launch_text("bringup.launch.py")
    assert "robot.launch.py" in text
...
# ---------------------------------------------------------------------------
# Schema parameter existence tests (robot.launch.py owns _PARAMS)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Scene resolution tests (robot.launch.py owns _scene_defaults)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Forwarding tests (robot.launch.py forwards to subsystems)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Structural tests (check appropriate Layer 2 files)
# ---------------------------------------------------------------------------

def test_bringup_launch_uses_tf_gate_node_instead_of_direct_sensor_delay():
    text = _launch_text("gas_simulation.launch.py")
...
def test_robot_launch_includes_all_subsystems():
    """Verify robot.launch.py includes all 5 subsystems."""
    text = _launch_text("robot.launch.py")
    assert "sim.launch.py" in text
    assert "nav2.launch.py" in text
    assert "gas_simulation.launch.py" in text
    assert "tracking.launch.py" in text
    assert "visualization.launch.py" in text


def test_gas_simulation_launch_includes_simplified_and_gaden():
    """Verify gas_simulation.launch.py has both simplified and GADEN paths."""
    text = _launch_text("gas_simulation.launch.py")
    assert "gas_field_node" in text
    assert "gaden_environment" in text
    assert "gaden_player" in text
    assert "gaden_sensor_gate_node" in text
    assert "gaden_adapter_node" in text


def test_tracking_launch_includes_bt_runner_and_particle_filter():
    """Verify tracking.launch.py has bt_node_runner and particle_filter_node."""
    text = _launch_text("tracking.launch.py")
    assert "bt_node_runner" in text
    assert "particle_filter_node" in text
    assert "nav2_startup_gate_node" in text
    assert "activate_localization" in text


def test_visualization_launch_includes_rviz():
    """Verify visualization.launch.py has RViz."""
    text = _launch_text("visualization.launch.py")
